...
function humanlikedev_framework_register_elementor_locations( $elementor_theme_manager ) {

	$elementor_theme_manager->register_location( 'header' );
	$elementor_theme_manager->register_location( 'footer' );

}
add_action( 'elementor/theme/register_locations', 'humanlikedev_framework_register_elementor_locations', 10, 1 );

function humanlikedev_framework_add_elementor_widget_categories( $elements_manager ) {
	
	$elements_manager->add_category(
		'humanlike-elements',
		array(
			'title' => 'Humanlike Elements'
		)
	);

}
add_action( 'elementor/elements/categories_registered', 'humanlikedev_framework_add_elementor_widget_categories', 10, 1 );
 ...
 * Icon Shortcode
 */
if(!( function_exists('humanlikedev_typed_text_shortcode') )) {
	function humanlikedev_typed_text_shortcode( $atts ) {
	...
	add_shortcode( 'humanlike_typed_text', 'humanlikedev_typed_text_shortcode' );
}

/**
 * Login Shortcode
 */
if(!( function_exists('humanlikedev_login_shortcode') )) {
	function humanlikedev_login_shortcode( $atts ) {
		$find = array(
	...
	add_shortcode( 'humanlike_login', 'humanlikedev_login_shortcode' );
}

/**
 * Stars Shortcode
 */
if(!( function_exists('humanlikedev_stars_shortcode') )) {
	function humanlikedev_stars_shortcode( $atts ) {
	    ...
	    $output = '<div class="d-flex mr-2">';
	    	...
	    	if ( strpos( $values['number_of_stars'], "." ) ) {
		      	...
		      	$html = humanlikedev_svg_icons_pluck( 'Star', $class = 'icon bg-warning' );
		      	$output .= str_repeat( $html, $whole_number );
		      	$output .= humanlikedev_svg_icons_pluck( 'Half Star', $class = 'icon bg-warning' );
		    } else {
	...
	add_shortcode( 'humanlike_stars', 'humanlikedev_stars_shortcode' );
}

/**
 * Video Lightbox Shortcode
 */
if(!( function_exists('humanlikedev_video_lightbox_button_shortcode') )) {
	function humanlikedev_video_lightbox_button_shortcode( $atts ) {
			...
			$output = '
				<a data-fancybox href="'. esc_url( $values['media_url'] ) .'" class="d-flex align-items-center">
					<span class="btn btn-primary btn-round btn-sm">
						'. humanlikedev_svg_icons_pluck( 'Play', 'icon' ) .'
					</span>
	...
	add_shortcode( 'humanlike_video_lightbox_button', 'humanlikedev_video_lightbox_button_shortcode' );
}

/**
 * Icon Button Shortcode
 */
if(!( function_exists('humanlikedev_icon_button_shortcode') )) {
	function humanlikedev_icon_button_shortcode( $atts ) {
    	...
    	if( 'large' == $values['button_style'] ) {
 			...
 			$output = '<a href="'. esc_url( $values['url'] ) .'" class="btn btn-lg btn-primary mx-sm-2 mb-3 mb-sm-0"">
            	'. humanlikedev_svg_icons_pluck( $values['icon_name'], 'icon' ) .'
	...
	add_shortcode( 'humanlike_icon_button', 'humanlikedev_icon_button_shortcode' );
}

/**
 * Icon Shortcode
 */
if(!( function_exists('humanlikedev_icon_shortcode') )) {
	function humanlikedev_icon_shortcode( $atts ) {
	    ...
	    $output = humanlikedev_svg_icons_pluck( $values['icon_name'], 'icon ' . $values['class'] );
	     
	    return $output;
	 
	}
	add_shortcode( 'humanlike_icon', 'humanlikedev_icon_shortcode' );
}

/**
 * Icon Shortcode
 */
if(!( function_exists('humanlikedev_counter_shortcode') )) {
	function humanlikedev_counter_shortcode( $atts ) {
	...
	add_shortcode( 'humanlike_counter', 'humanlikedev_counter_shortcode' );
}

/**
 * Icon Shortcode
 */
if(!( function_exists('humanlikedev_all_icons_shortcode') )) {
	function humanlikedev_all_icons_shortcode( $atts ) {

		$all_icons = humanlikedev_get_svg_icons();

		$output = '<div class="row justify-content-center text-center">';

			foreach ( array_slice( $all_icons, 1 ) as $key => $value) {
				$output .= '<div class="col-lg-2 col-md-2 mb-4">'. humanlikedev_svg_icons_pluck( $key, 'icon icon-lg bg-primary clearfix' ). '<span class="small clearfix">'. $key .'</span></div>';
			}	
	...
	add_shortcode( 'humanlike_all_icons', 'humanlikedev_all_icons_shortcode' );
}

/* SOCIAL SHARING */
if(!( function_exists('humanlikedev_social_sharing') )) {
	function humanlikedev_social_sharing() {
