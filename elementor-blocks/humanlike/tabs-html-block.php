...
class Widget_HumanlikeDev_Tabs_HTML_Block extends Widget_Base {
	
	//Return Class Name
	public function get_name() {
		return 'humanlikedev-tabs-html-block';
	}
				    			...
				    			echo '
						      		<li class="nav-item">
						        		<a class="nav-link btn btn-light p-2 text-left '; if( 0 == $i ) { echo 'active'; } echo '" href="#'. sanitize_title( $item['title'] ) .'" data-toggle="tab" aria-controls="'. sanitize_title( $item['title'] ) .'" aria-selected="true" role="tab">
						        			'. humanlikedev_svg_icons_pluck( $item['icon'], 'icon bg-primary' ) .'
						          		'. $item['title'] .'</a>
						     		 </li>';
...
Plugin::instance()->widgets_manager->register_widget_type( new Widget_HumanlikeDev_Tabs_HTML_Block() );