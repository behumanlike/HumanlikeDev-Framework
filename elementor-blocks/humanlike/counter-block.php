...
class Widget_HumanlikeDev_Counter_Block extends Widget_Base {
	
	//Return Class Name
	public function get_name() {
		return 'humanlikedev-counter-block';
	}
		...
		if( 'regular' == $settings['layout'] ) {
			...
			if( '0' !== $settings['icon'] ) {
				...
				echo '<div class="d-flex flex-md-column">
						...
						'. humanlikedev_svg_icons_pluck( $settings['icon'], 'icon bg-'.$settings['colour'] ) .'
						</div>'; 
...
Plugin::instance()->widgets_manager->register_widget_type( new Widget_HumanlikeDev_Counter_Block() );