 ...
 * The Shortcode
 */
function humanlikedev_flex_list_shortcode( $atts, $content = null ) {
	extract( 
		shortcode_atts( 
			array(
				'custom_css_class' => ''
...
add_shortcode( 'insight_flex_list', 'humanlikedev_flex_list_shortcode' );

/**
 * The Shortcode
 */
function humanlikedev_flex_list_content_shortcode( $atts, $content = null ) {
	extract( 
		shortcode_atts( 
			array(
				'title' 			=> '',
...
add_shortcode( 'insight_flex_list_content', 'humanlikedev_flex_list_content_shortcode' );

// Parent Element
function humanlikedev_flex_list_shortcode_vc() {
	vc_map( 
		array(
			"icon" => 'insight-vc-block',
		    'name'                    => esc_html__( 'Flex List' , 'humanlikedev' ),
		    ...
		    "category" => esc_html__('Insight WP Theme', 'humanlikedev'),
		    'params'          => array(
		    	array(
		    		"type" => "textfield",
...
add_action( 'vc_before_init', 'humanlikedev_flex_list_shortcode_vc' );

// Nested Element
function humanlikedev_flex_list_content_shortcode_vc() {

	$icons = array('Install HumanlikeDev Framework' => 'Install HumanlikeDev Framework');
		
	if( function_exists('humanlikedev_get_icons') ){
		$icons = humanlikedev_get_icons();	
	}

	vc_map( 
		array(
			"icon" => 'insight-vc-block',
		    'name'            => esc_html__('Flex List Item', 'humanlikedev'),
		    ...
		    'params'          => array(
		    	array(
					"type"       => "dropdown",
					"heading"    => __( "Item Type", 'humanlikedev' ),
					"param_name" => "layout",
					"value"      => array(
				...
				array(
					"type" => "humanlikedev_icons",
					"heading" => esc_html__( "Click an Icon to choose", 'humanlikedev' ),
					"param_name" => "icon",
					"value" => $icons,
		    	...
		    	array(
		    		"type" => "textfield",
		    		"heading" => esc_html__("Title", 'humanlikedev'),
		    		"param_name" => "title",
		    		'holder' => 'div'
		    	),
	            array(
	            	"type" => "textarea_html",
	            	"heading" => esc_html__("Block Content", 'humanlikedev'),
	            	"param_name" => "content"
	            ),
	            array(
		    		"type"       => "attach_image",
		    		"heading"    => __( "Icon Image", 'humanlikedev' ),
		    		"param_name" => "image",
		    		"description" => __( 'Will override icon font if an image is set - Only used in the "Icon/Image & Title Left, Arrow Right" layout.', 'humanlikedev' ),
		    	),
	            array(
					"type"        => "vc_link",
					"heading"     => __( "Link", 'humanlikedev' ),
					"param_name"  => "link"
				),
	            array(
	            	"type" => "textfield",
...
add_action( 'vc_before_init', 'humanlikedev_flex_list_content_shortcode_vc' );
