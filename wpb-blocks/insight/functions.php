...
if(!( function_exists( 'humanlikedev_vc_page_template' ) )){
	function humanlikedev_vc_page_template( $template ){
	...
	add_filter( 'template_include', 'humanlikedev_vc_page_template', 100, 1 );
}


if( !( function_exists('humanlikedev_icons_settings_field') ) && function_exists('vc_set_as_theme') ){
	function humanlikedev_icons_settings_field( $settings, $value ) {
		
		$icons = $settings['value'];
		
		$output = '<a href="#" id="humanlikedev-icon-toggle" class="button button-primary button-large">Show/Hide Icons</a><div class="humanlikedev-icons"><div class="humanlikedev-icons-wrapper">';
		foreach( $icons as $icon ){
			$active = ( $value == $icon) ? ' active' : '';
			$output .= '<i class="icon '. $icon . $active .'" data-icon-class="'. $icon .'"></i>';
		}
		$output .= '</div><input name="' . esc_attr( $settings['param_name'] ) . '" class="wpb_vc_param_value wpb-textinput humanlikedev-icon-value ' .
		esc_attr( $settings['param_name'] ) . ' ' .
		esc_attr( $settings['type'] ) . '_field" type="text" value="' . esc_attr( $value ) . '" />' . '</div>';
		
	   return $output;
	}
	vc_add_shortcode_param( 'humanlikedev_icons', 'humanlikedev_icons_settings_field' );
}

if( !( function_exists('humanlikedev_vc_alter_class_field_desc') ) && function_exists('vc_set_as_theme') ){
	function humanlikedev_vc_alter_class_field_desc() {
	 
	    $vc_column_text_new_params = array(
	         
	        array(
	        	"type"        => "textfield",
	    		"heading"     => __( "Extra class name", 'humanlikedev' ),
	            'param_name' => 'el_class',
	            'description' => __( 'Style particular content element differently - add a class name and refer to it in custom CSS, or you can use one of the themes pre-defined classes - <strong>bg-dark</strong>, <strong>bg-white</strong>, <strong>bg-gradient</strong>, <strong>bg-gradient-1</strong>, <strong>bg-gradient-2</strong>', 'humanlikedev' ),
	        ),      
	...
	add_action( 'vc_after_init', 'humanlikedev_vc_alter_class_field_desc' );
}
 ...
 */
if( !( function_exists('humanlikedev_vc_add_att') ) && function_exists('vc_set_as_theme') ){
	function humanlikedev_vc_add_attr(){
		...
		vc_add_param('humanlikedev_post', $attributes);
		...
		vc_add_param('humanlikedev_team', $attributes);
		...
		vc_add_param('humanlikedev_testimonial', $attributes);
		...
		vc_add_param('humanlikedev_portfolio_feed', $attributes);
		
	}
	add_action('init', 'humanlikedev_vc_add_attr', 999);
}
