 ...
 * The Shortcode
 */
function humanlikedev_portfolio_feed_shortcode( $atts ) {
...
add_shortcode( 'humanlikedev_portfolio_feed', 'humanlikedev_portfolio_feed_shortcode' );
/**
 * The VC Functions
 */
function humanlikedev_portfolio_feed_shortcode_vc() {
	vc_map( 
		array(
			"icon" => 'insight-vc-block',
			"name" => esc_html__("Portfolio Feed", 'humanlikedev'),
			"base" => "humanlikedev_portfolio_feed",
			"category" => esc_html__('Insight WP Theme', 'humanlikedev'),
			'description' => 'Show post posts with layout options.',
			"params" => array(
				array(
					"type" => "textfield",
					"heading" => esc_html__("Show How Many Projects?", 'humanlikedev'),
					"param_name" => "pppage",
					"value" => '4'
				),
				array(
					"type" => "dropdown",
					"heading" => esc_html__("Display Type", 'humanlikedev'),
					"param_name" => "layout",
					"value" => array_flip( humanlikedev_get_portfolio_layouts() ),
				),
				array(
					"type" => "textfield",
...
add_action( 'vc_before_init', 'humanlikedev_portfolio_feed_shortcode_vc');